...
# Choose for Sector
query = 'SELECT DISTINCT SectorD, ClientesD FROM Clientes WHERE Time >= "18:00:00" AND FechaC >= ? AND FechaC <= ? ORDER BY ClientesD, SectorD;'
params = [s, e]
...
if not sector and not cliente:
    query = 'SELECT "index", Fecha, SectorD, ClientesD, Cluster, Active_energy, FechaC, Anomalo FROM Clientes WHERE Time >= "18:00:00" AND FechaC >= ? AND FechaC <= ?;'
    params = [s, e]
    ...
    query = 'SELECT "index", Fecha, SectorD, ClientesD, Cluster, Active_energy, FechaC, Anomalo FROM Clientes WHERE Time >= "18:00:00" AND FechaC >= ? AND FechaC <= ? AND SectorD IN ({});'.format(', '.join(["'{}'".format(v) for v in sector]))      
    params = [s, e] 
    #df = pd.read_sql_query (query, con, params=params)
    #df['fecha_ymd'] = df.iloc[:, 1].str[:10]
    df["fecha_ymd"] = pd.to_datetime(df["fecha_ymd"]) 
    ...
    query = 'SELECT "index", Fecha, SectorD, ClientesD, Cluster, Active_energy, FechaC, Anomalo FROM Clientes WHERE Time >= "18:00:00" AND FechaC >= ? AND FechaC <= ? AND ClientesD IN ({});'.format(', '.join(["'{}'".format(v) for v in cliente]))      
    params = [s, e] 
    df = pd.read_sql_query (query, con, params=params)
    #df['fecha_ymd'] = df.iloc[:, 1].str[:10]
    #df["fecha_ymd"] = pd.to_datetime(df["fecha_ymd"]) 
